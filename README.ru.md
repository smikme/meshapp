...
Карта показывает узлы с координатами и сохранённые маршруты. Поддерживаются интерактивные тайлы OpenStreetMap, локальный HTTP-кэш и заранее подготовленный офлайн-каталог с тайлами в формате `z/x/y.png|jpg|jpeg`.

На карте есть поиск, фильтры, переход к своему устройству, обзор всех узлов с координатами, ночной режим, измерение расстояний и выделение прямоугольной области. MeshApp запрашивает только тайлы текущей видимой области при интерактивной работе и не выполняет массовую загрузку областей OpenStreetMap. Для работы без сети подключите отдельно полученный каталог тайлов из источника, условия которого разрешают офлайн-использование.

Другого интерактивного провайдера можно выбрать при запуске через JVM-параметры `meshapp.map.tileSource.url`, `meshapp.map.tileSource.id`, `meshapp.map.tileSource.attribution`, `meshapp.map.tileSource.minZoom` и `meshapp.map.tileSource.maxZoom`. URL-шаблон должен содержать `{z}`, `{x}` и `{y}`. Конфигурация времени запуска никогда не включает массовую загрузку.
...
Для Meshtastic 2.8.0 и новее MeshApp поддерживает компактную начальную базу
нод с последующей фоновой передачей координат и телеметрии. Для старых прошивок
сохраняется прежний сценарий подключения.
...
Настройки прошивки 2.8 включаются только когда целевая нода сообщает версию
2.8.0 или новее. К ним относятся допустимые сочетания региона и пресета,
политика подписей пакетов, Mesh Beacon и ограничение длинного имени 24 байтами
UTF-8.
