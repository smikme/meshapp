...
MeshApp нужен для повседневной работы с Meshtastic и MeshCore с компьютера: общаться, смотреть сеть, менять настройки и разбирать, что происходит в эфире. В приложении можно:

- переписываться в mesh-каналах и личных диалогах, отвечать на сообщения, ставить реакции и видеть ACK/NAK;
- смотреть ноды сети: искать, фильтровать, отмечать избранные и игнорируемые, открывать карточку ноды, запускать traceroute и запрашивать `NodeInfo`;
- работать с картой: онлайн- и офлайн-тайлы, ноды, сохранённые traceroute-маршруты, измерения и скачивание области для офлайна;
- следить за телеметрией устройства и сети, смотреть историю на графиках или в таблице;
- менять конфигурацию Meshtastic-устройства, включая каналы и расширенные поля;
- подключаться по TCP, USB/Serial или BLE и выбирать протокол: Meshtastic, MeshCore KISS или MeshCore Companion;
- смотреть логи и LoRa-пакеты, экспортировать захват для разбора;
- писать и запускать Lua-скрипты: есть редактор, отладчик, автозапуск, KV-хранилище, боты и магазин скриптов;
- хранить всё локально: сообщения, телеметрию, ноды, traceroute, скрипты и журнал пакетов;
- запускать терминальный режим, если JavaFX-интерфейс не нужен.
