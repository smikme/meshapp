/**
 * meshapp-ble: Linux BLE library for MeshApp (BlueZ/sd-bus)
 *
 * Flat C API wrapping BlueZ D-Bus operations via sd-bus.
 * Uses AcquireNotify/AcquireWrite for fd-based GATT I/O.
 * Called from Java via JNA. All functions are thread-safe.
 *
 * Return values: 0 = success, negative = error.
 * Callbacks fire from the worker thread; caller handles synchronization.
 */
...
/** Initialize sd-bus connection and BlueZ subsystem. Call once before any other function. */
MESHBLE_API int meshble_init(void);

/** Release all resources and close D-Bus connection. Safe to call multiple times. */
 ...
 * @return 0 on success, -4 when the BLE bond is not authenticated/MITM, otherwise negative on error
 */
