...
#### IDE для Lua-разработки

MeshApp IDE — встроенная рабочая среда для написания, проверки и упаковки Lua-автоматизации прямо внутри клиента. Базовые функции:

- **Рабочее пространство скриптов** — создание скриптов, открытие редактора, запуск и остановка, включение автозапуска, редактирование метаданных и удаление из карточек
- **Редактор кода** — подсветка синтаксиса, номера строк, автоотступы, проверка синтаксиса, сохранение кода в локальную БД и контекстное автодополнение для `mesh.*`, стандартных Lua-библиотек и найденных локальных символов
- **Консоль выполнения** — вывод сообщений скрипта, статусов запуска/остановки, синтаксических и runtime-ошибок при тестировании на активной или настроенной ноде
- **Отладка** — точки останова в поле строк, запуск debug-сессии, продолжение выполнения, пошаговое выполнение и просмотр локальных/global переменных на паузе
- **KV-хранилище** — просмотр и редактирование изолированной key-value базы выбранного скрипта: поиск, создание, обновление и удаление записей
- **Пакетирование скриптов** — импорт и экспорт файлов `.meshapp-script.json` с кодом и метаданными
- **Магазин скриптов** — открытие каталога, фильтр по типу, установка, обновление и удаление скриптов
- **Sandbox-среда** — скриптам доступны разрешённые Lua-функции и namespace `mesh`; небезопасные API вроде `io`, `os`, `debug`, `package`, `require`, `dofile`, `loadfile` и `luajava` отключены
- **Точки входа Mesh API** — `on_message(msg)` для эфирных ботов, `on_command(command)` для automation-ботов и асинхронные callbacks `on_node_selected(event)`, `on_traceroute(event)`, `on_node_info(event)`. Полное API описано в [docs/lua-api.ru.md](docs/lua-api.ru.md)

#### Возможности Lua-скриптов
